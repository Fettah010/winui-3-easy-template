...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        // Feature-gated chrome: the tray button only exists with --tray.
        // (Visibility, not XAML removal — the template engine skips markers
        // in markup files, so flags gate UI at runtime via AppFeatures.)
        TrayButton.Visibility = AppFeatures.Tray ? Visibility.Visible : Visibility.Collapsed;
    ...
    private void ApplyNavLocalization()
    {
        try
        {
            ...
            if (RootNavigationView.SettingsItem is NavigationViewItem settingsItem)
            {
                ...
                // Stable hook for UI automation tests: the built-in
