...
- **2026-09-22 - Formixa cleanup pass: scaffold starts clean, runtime hardens.** Pain-log 1-25 from scaffolding a real app. Sample content gets an inverse tool (Scripts/remove-sample-content.ps1: dirty-tree guard + rollback, XAML + code-behind + 3-language keys, then build + test) instead of a manual checklist; user strings go neutral (no 'template' prose; FirstRun appends a per-UpdateMode line so store scaffolds never advertise GitHub updates); repo/license URLs resolve at runtime from ProductConfiguration via AppMetadata (XAML carries no hardcoded repo; init-template -LicenseName/-LicenseUrl rewrites the defaults). add-page.ps1 activates the dormant nested template via a temp copy (repo stays clean) and falls back to the <devtem:nav-items> end marker for restyled shells. DialogHelper owns the uncollapse-around-ShowAsync pattern with a reentrancy gate; FilePickerService races a 60s timeout and degrades to cancel; XamlSymbolAuditTests pins every Symbol against the known-good set; UnitConversion + ChangeEpoch bottle the PDF/canvas lessons with A4-pinned tests; ServiceLocatorTests resolves EVERY VM; run-app.ps1 prints binary time vs HEAD and About shows BuildCommit. Scaffold version reset stays one documented command (bump-version -Version 0.0.1): dotnet-new runs no post-actions (IDE-only), so automation is impossible by engine design - README/AGENTS say so instead of pretending otherwise.
- **2026-09-22 — Encoding discipline: scripts use explicit UTF-8 IO.** bump-version.ps1 read UTF-8 csprojs via bare Get-Content (system-codepage decode) and saved via XmlDocument.Save (UTF-8 encode), doubling non-ASCII on every run: Templates/Project/__SafeName__.csproj grew 5KB -> 148MB across releases (em-dash seed in a comment) and GitHub refused the push (GH001). Fixed by reconstructing the file from current structure + the pristine baseline comment (ASCII hyphen now), switching bump-version/add-page to [System.IO.File] UTF-8 IO, and adding Tests/Services/RepoHygieneTests.cs (U+FFFD scan + 100KB csproj cap). Rule: no bare Get/Set-Content on repo files, ASCII-only in tooling comments, non-ASCII lives in loc dictionaries only.
- **2026-09-22 — Advancement Phase A shipped: fail-closed basic updates, import caps, quoting pins, secret/PII tests, basic release path, identity validation.** BasicGithubUpdateService verifies SHA-256 against the sibling .exe.sha256 (missing/mismatch refuses, partial deleted, verified-path gate at launch) with 30s check + 30min download budgets; Scripts/publish-basic.ps1 produces the pair (self-contained single-file needs -p:EnableMsixTooling=true) and template.json excludes it outside basic. SettingsBackupService caps imports at 1MB with a schema version (future MAJOR refused, legacy accepted). Registry command values gained pure builders + spaced-path tests. RepoHygieneTests now fails on secret-adjacent AppLog calls; crash status keys pinned; export bundles scrub user paths; docs/feature-guides/crash.md records the posture (excluded when !crash). init-template -Validate checks metadata-vs-csproj-vs-manifest per scaffold via the matrix. Two matrix contracts repaired along the way: repo-literal assertion moved to ProductConfiguration (AppMetadata resolves at runtime since 0.0.27), scaffold README keeps a compact Starter-profiles block the matrix pins.
- **2026-09-22 — Advancement Phase B shipped (B1-B4, B6-B9; B5 static half).** B1: one responsive mechanism (code-behind) — Settings + Diagnostics VSM blocks deleted (Diagnostics was VSM-only, not code-behind as the plan assumed); TEMPLATE-GUIDE pins the rule. B2: KeyboardShortcuts resolver (pure, tested) + MainWindow KeyDown/PointerPressed wiring (Alt+Left/Right, X1/X2, Ctrl+comma, Esc to DismissAsync). B3: toast host + Settings status Polite, error cards Assertive (LiveRegionTests); the per-second log tail stays un-announced (a live region on a 1s feed is screen-reader spam). B4: BufferedLogEvent hoisted top-level so Diagnostics rows compile (x:DataType + OneTime); the item template has no list, nothing to convert. B5 static: fixed-height/hardcoded-color audit clean, TextScalingAuditTests pins it; the 200%/HC/EN-ES screenshot matrix needs a Live-UI desktop (open, noted on the box). B6: devtem-list-details (selection + empty state, x:DataType rows, x:Load states) + add-page -Kind + matrix list smoke; the matrix caught two real pre-commit bugs (x:Load needs x:Name, Assert.HasCount). B7: button rule codified (Primary verb, dismiss word) + DialogButtonLanguageTests; whats-new title/button localized. B8: whats-new capped (1200) with releases link, About recall entry, once-per-version pinned; no quiet hours (security updates prompt regardless). B9: failed checks keep last-good + sticky flag drives the Home error card; failed downloads re-arm CanDownload; wizard location validates inline (Next/Complete blocked). Per-state screenshots stay manual.
- **2026-09-22 — C3 spike verdict: IHost DECLINED, question closed.** Isolated spike (net10 console, equivalent registration shape: singletons + transient VMs + HttpClient): `Host.CreateDefaultBuilder().Build()` vs bare `ServiceCollection.BuildServiceProvider()`. Warm median: 2.3ms vs 0.1ms (delta ~2ms). Fresh-process cold first-call (incl. JIT + assembly load, 5 launches each): host ~139ms vs bare ~29ms (delta ~110ms). The old "costs seconds" claim was wrong — corrected to the measured numbers in `ServiceLocator` docs. Verdict: keep the bare container. The host's delta buys nothing needed: DI (container), logging (Serilog facade + sinks), configuration (`ProductConfiguration` + `DEVTEM_*` env, no JSON files wanted), lifetime (the update loop is UI-thread-affine by design — an `IHostedService` would add thread-hopping + startup-ordering risk against the P0-4 deferred-init rule for zero functional gain). C4/C5 proceed host-free per plan. Spike ran outside the repo (nothing committed); these numbers are the permanent answer.
- **2026-09-22 — C6 evaluation: auth template DEFERRED, no code.** Broker (WAM) vs embedded: broker gives SSO + conditional-access + no credential handling, but unpackaged WinUI must register a package SID + `ms-appx-web` redirect with the broker — doable, entitlement-adjacent, and per-app Entra registration the template cannot ship; embedded browser (system browser loopback) works everywhere with zero packaging questions but no SSO and a weaker phishing story. Token cache per distribution: portable/Velopack unpackaged = DPAPI file under the app data dir (`AppPaths`); MSIX packaged = same shape under `LocalFolder`; either way MSAL's cache helper, never custom storage. A future `--auth` flag would include: MSAL.NET ref + `AuthService` (acquire/clear, broker-first with embedded fallback) + login/logout UI + cache wiring per distribution, and exclude everything when off (no identity SDK in the default scaffold — dependency + privacy weight). Verdict: defer until a real consumer asks (flag discipline P3-4; identity providers vary too much to template blind). Revisit with a spike against a real tenant, not a guess.
- **2026-09-22 — D2 diet verdicts: trim declined, per-combo ceilings deferred.** `PublishTrimmed`: declined for WinUI (XAML + WinRT projections break under trimming — same class as the `PublishAot` decline; track annually with it). No owned non-WinUI assembly is big enough to trim alone (app code is KBs; the 268MB is SDK + Velopack + SQLite native). Per-combo weight ceilings: deferred to a nightly/tag job — 21 Release publishes at ~4min each ≈ 84min, over the 30min matrix budget (links E1's cost-control work). The single-config gate + release trend carry the weight story until then. Dead Toolkit refs (`Animations`, `Helpers` — nothing used them) removed anyway: 267.8MB vs 268.2MB (-0.2%; transitives remain via Controls).
- **2026-09-23 — D3 cold flame + first armed-budget triage.** Debug dev box, three launches: cold (first of day) splash 1451 / services 1715 / window 1964ms; warm splash ~300 / services ~320 / window ~500ms. P0-4 holds (ctor = chrome + navigation; the single-instance listener thread moved to deferred init — the only creep found). The armed `Report` fired correctly on the cold splash (1451 > 800×1.15). Triage: cold splash is loader/JIT-dominated (app-code services phase only ~264ms of it; warm proves the code path is fast), so the 800 budget was unmeasured aspiration — re-baselined to 1600 with this reason (the sanctioned mute procedure), window 2500 holds (cold 1964). Warm numbers are the regression floor to watch.
- **2026-09-23 — Advancement Phase D shipped (budgets with teeth).** D1: `StartupBudgets.Report` logs breaches as launch errors + shape anti-mute test + CI `weight` job with `-FailOnJump` (local gate proven +0.1%). D2: dead Toolkit refs removed (267.8MB, -0.2%), assets verified, trend rule in WORKFLOW. D3: P0-4 holds (listener thread moved to deferred); cold flame 1451/1715/1964, warm ~300/~320/~500; splash re-baselined 800→1600 with reason (loader/JIT-dominated). D4: bundle export off UI thread, DB 30s timeout + 2s tripwire, tail caps pinned by 2MB test; no user-image path exists. D5: splash was already honest; added the 1.0 completion milestone.
- **2026-09-23 — Advancement Phase E shipped (mechanics, CI, docs).** E1: `-Profile Fast` (allon/alloff/msixapp) for PRs, full matrix on main/nightly/tags, tier table in WORKFLOW. E2: CI auto-retry once with both-runs evidence + display note (in-test retry/screenshots already existed). E3: `DocsDriftTests` (csproj pointer, version allowlist, generic branch table, relational page mentions); rotting example removed. E4: guide split byte-exact (539 → 250 daily + GETTING-STARTED + MAINTAINERS), cross-links updated, new docs frozen for identity. E5: release notes filled, `devtem-list-details` added to the nupkg (packaging missed it — caught by audit), pack verified end-to-end (277 entries, install + resolve + update-check). E6: version-reset stays one command (engine unchanged). E7: updates × distribution truth table in `distribution-dual.md`. Manual remainders: VS dialog screenshots (release PR), B5/B9 screenshot passes, Narrator.
- **2026-09-23 — E6 re-verdict: version-reset stays one command.** The dotnet-new engine still runs no post-actions (IDE-only by design, unchanged) — scaffolded apps keep inheriting the template version with the single documented `bump-version -Version 0.0.1` reset. If the engine ever gains post-actions, prefer a `version` symbol (default `0.0.1`) over scripts; no custom post-scaffold runner (worse UX than flags).
- **2026-09-23 - F1 DataGrid verdict refined (shape adopted, ContentGrid declined).** `devtem-datagrid` ships on `CommunityToolkit.WinUI.UI.Controls.DataGrid` 7.1.2: the only WinUI 3 DataGrid ever published (no 8.x; toolkit authors bless side-by-side use - it carries no toolkit dependencies). Verified restoring + compiling on net10. The reference lands only when a grid page is added (`add-page -Kind grid`), never in the base scaffold; the csproj anchor is the full versioned line so a Toolkit bump fails loudly and forces re-checking the pin. Columns bind classic (`{Binding}` - DataGrid columns are not compiled-binding targets); the mapping is pinned by a row-property test + throwing `ApplySort`. ContentGrid declined: a reflowing grid is a list, and list exists.
- **2026-09-23 - F2 suspend/activation spike: CLOSE, no code (C3-class).** Windows App SDK apps are running-or-not-running (Microsoft applifecycle docs) - no suspend/resume state, so a Suspending handler would be dead code packaged or not. The documented replacement (main-window `Closed`) is already our pattern: settings flush + background-loop cancel on `Closed`. Activation is covered (protocol cold-start + second-instance redirect + toast action); file activation waits for a consumer (same rule as C6). `ExtendedExecution` unnecessary (nothing to finish during a suspend that never comes).
